 ...
 *         Shakespeare, downloaded from Project Gutenberg. Training on other
 *         text sources should be relatively easy to implement.
 *
...
public class GravesLSTMCharModellingExample {
	...
	final int lstmLayerSize = 200; // Number of units in each GravesLSTM
				       // layer
	final int miniBatchSize = 32; // Size of mini batch to use when training
	final int exampleLength = 1000; // Length of each training example
					// sequence to use. This could certainly
					// be
	// increased
	final int tbpttLength = 50; // Length for truncated backpropagation
				    // through time. i.e., do parameter updates
	// ever 50 characters
	final int numEpochs = 1; // Total number of training epochs
	final int generateSamplesEveryNMinibatches = 10; // How frequently to
							 // generate samples
							 // from the network?
							 // 1000
	// characters / 50 tbptt length: 20 parameter updates per
	// minibatch
	final int nSamplesToGenerate = 4; // Number of samples to generate after
					  // each training epoch
	final int nCharactersToSample = 300; // Length of each sample to
					     // generate
	final String generationInitialization = null; // Optional character
						      // initialization; a
						      // random character is
						      // used
	// if null
	...
	final int nOut = iter.totalOutcomes();
	...
	final MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder()
		...
		.layer(1,
			new GravesLSTM.Builder().nIn(lstmLayerSize).nOut(lstmLayerSize).activation(Activation.TANH)
				.build())
		.layer(2, new RnnOutputLayer.Builder(LossFunction.MCXENT).activation(Activation.SOFTMAX) // MCXENT +
			// softmax for
			// classification
	...
	final Layer[] layers = net.getLayers();
	int totalNumParams = 0;
	for (int i = 0; i < layers.length; i++) {
	    final int nParams = layers[i].numParams();
	...
	for (int i = 0; i < numEpochs; i++) {
		...
		if ((++miniBatchNumber % generateSamplesEveryNMinibatches) == 0) {
		    ...
		    final String[] samples = sampleCharactersFromNetwork(generationInitialization, net, iter, rng,
     ...
     * @param net                MultiLayerNetwork with one or more GravesLSTM/RNN
     *                           layers and a softmax output layer
     * @param iter               CharacterIterator. Used for going from indexes back
     *                           to characters
     */
    private static String[] sampleCharactersFromNetwork(String initialization, final MultiLayerNetwork net,
	    final CharacterIterator iter, final Random rng, final int charactersToSample, final int numSamples) {
	...
	final INDArray initializationInput = Nd4j.zeros(numSamples, iter.inputColumns(), initialization.length());
	final char[] init = initialization.toCharArray();
	for (int i = 0; i < init.length; i++) {
	    final int idx = iter.convertCharacterToIndex(init[i]);
	    for (int j = 0; j < numSamples; j++) {
		initializationInput.putScalar(new int[] { j, idx, i }, 1.0f);
	    }
	}

	final StringBuilder[] sb = new StringBuilder[numSamples];
	...
	output = output.tensorAlongDimension(output.size(2) - 1, 1, 0); // Gets
									// the
									// last
									// time
									// step
	...
	for (int i = 0; i < charactersToSample; i++) {
	    ...
	    // output
	    final INDArray nextInput = Nd4j.zeros(numSamples, iter.inputColumns());
	    // Output is a probability distribution. Sample from this for each
	    // example we
	    // want to generate, and add it to the new input
	    for (int s = 0; s < numSamples; s++) {
		final double[] outputProbDistribution = new double[iter.totalOutcomes()];
		for (int j = 0; j < outputProbDistribution.length; j++) {
		    outputProbDistribution[j] = output.getDouble(s, j);
		}
		final int sampledCharacterIdx = sampleFromDistribution(outputProbDistribution, rng);

		nextInput.putScalar(new int[] { s, sampledCharacterIdx }, 1.0f); // Prepare
										 // next
										 // time
										 // step
										 // input
		sb[s].append(iter.convertIndexToCharacter(sampledCharacterIdx)); // Add
										 // sampled
										 // character
										 // to
	    ...
	    output = net.rnnTimeStep(nextInput); // Do one time step of forward
						 // pass
	}

	final String[] out = new String[numSamples];
	for (int i = 0; i < numSamples; i++) {
	    out[i] = sb[i].toString();
	}
	return out;
     ...
     */
    public static int sampleFromDistribution(final double[] distribution, final Random rng) {
